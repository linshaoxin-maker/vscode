...
import { IChatContentPartRegistry } from '../chatContentPartRegistry.js';
...
export class ChatListItemRenderer extends Disposable implements ITreeRenderer<ChatTreeItem, FuzzyScore, IChatListItemTemplate> {
	...
	constructor(
		...
		@IChatContentPartRegistry private readonly contentPartRegistry: IChatContentPartRegistry,
	) {
		super();
	...
	private fireItemHeightChange(template: IChatListItemTemplate, measuredHeight?: number): void {
		...
		if (prevHeight && Math.abs(normalizedHeight - prevHeight) > 100) {
			this.logService.trace('[ChatListHeight] BIG height change:', prevHeight, '->', normalizedHeight, 'delta:', normalizedHeight - prevHeight);
		}
