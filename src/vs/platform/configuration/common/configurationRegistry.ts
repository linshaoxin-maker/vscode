...
export const EXTENSION_UNIFICATION_EXTENSION_IDS: Set<string> = new Set(product.defaultChatAgent?.extensionId ? [product.defaultChatAgent.extensionId, product.defaultChatAgent.chatExtensionId].filter(Boolean).map(id => id!.toLowerCase()) : []);
