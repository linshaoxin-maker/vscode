...
function packageTask(platform: string, arch: string, sourceFolderName: string, destinationFolderName: string, _opts?: { stats?: boolean }) {
	...
	const task = () => {
		...
		const electronConfig = {
			...
			// chipos: code.car (compiled macOS asset catalog) is not in this fork.
			// Upstream vscode keeps it generated by `actool` from Assets.xcassets,
			// which never made it into chip-os/coderust. Skip the optional asset —
			// dock icon falls back to code.icns, IDE functionality unaffected.
